   ...
   partial class PathHelpers {
      ...
      static class VariablePathHelpers {
         ...
         internal static bool IsAVariablePath(string pathStringNormalized, out IReadOnlyList<string> variables, out string failureReason) {
            ...
            switch (result) {
               default:
                  ...
                  if (indexOut - variableName.Length - PathHelpers.PathVariableBegin.Length - PathHelpers.PathVariableEnd.Length != 0) {
               ...
               switch (result) {
                  default:
                     ...
                     foreach (var variableNameAlreadyAdded in variablesList) {
                        if (String.Compare(variableNameAlreadyAdded, variableName, true) != 0) {  // true for ignoreCase!
                           continue;
                        }
                        bAlreadyAdded = true;
                        continue;
         ...
         private static TryGetNextVariableResult TryGetNextVariable(
            ...
            var indexBegin = pathStringNormalized.IndexOf(PathHelpers.PathVariableBegin, indexIn);
            if (indexBegin == -1) {
               indexOut = -1;
               variableName = null;
            ...
            var indexVariableNameBegin = indexBegin + PathHelpers.PathVariableBegin.Length;

            var indexvariableNameEnd = pathStringNormalized.IndexOf(PathHelpers.PathVariableEnd, indexVariableNameBegin);
            if (indexvariableNameEnd == -1) {
               indexOut = -1;
               variableName = null;
               failureReasonTmp = @"Found variable opening ""$("" without closing parenthesis, at position " + indexBegin.ToString() + ".";
            ...
            indexOut = indexvariableNameEnd + PathHelpers.PathVariableEnd.Length;
            failureReasonTmp = null;
            return TryGetNextVariableResult.VariableFound;
         }
         #endregion Determine if a string is a variable path and extract variables!




         #region ReplaceVariableWithValue used at resolving time
         //-------------------------------------------------------------------------
         //
         ...
         internal static string ReplaceVariableWithValue(string pathString, string variableNameToResolve, string variableValue) {
            ...
            var variableNameDecorated = PathHelpers.PathVariableBegin + variableNameToResolve + PathHelpers.PathVariableEnd;
         ...
         private static bool TryReplaceVariableWithValue(int index, string pathStringIn, string variableNameDecorated, string variableValue, out string pathStringOut) {
         ...
         internal static string GetParentDirectory(string path) {
            ...
            int index = path.LastIndexOf(MiscHelpers.DIR_SEPARATOR_CHAR);
            Debug.Assert(index >= 0);
